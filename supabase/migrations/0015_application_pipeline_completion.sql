...
property_stats AS (
  SELECT 
    ...
    COUNT(CASE WHEN a.sender_employee_id = ce.id THEN 1 END) as outgoing_announcements_count,
  ...
  GROUP BY p.id, p.name, p.location, ce.id
)
SELECT 
...
CREATE OR REPLACE FUNCTION public.get_tenant_onboarding_status(p_user_id uuid DEFAULT auth.uid())
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  WHERE converted_user_id = p_user_id
     OR (email = (SELECT email FROM auth.users WHERE id = p_user_id))
...
GRANT EXECUTE ON FUNCTION public.complete_onboarding_step(uuid, text, text, text, text, text) TO authenticated;
...
CREATE POLICY IF NOT EXISTS tenant_applications_insert_public
...
CREATE POLICY IF NOT EXISTS tenant_applications_caretaker_select
  ON public.tenant_applications
  FOR SELECT
  TO authenticated
  USING (
...
CREATE POLICY IF NOT EXISTS tenant_applications_caretaker_update
  ON public.tenant_applications
  FOR UPDATE
  TO authenticated
  USING (
...
CREATE POLICY IF NOT EXISTS tenant_applications_admin_select
  ON public.tenant_applications
  FOR SELECT
  TO authenticated
  USING (
...
CREATE POLICY IF NOT EXISTS tenant_applications_admin_update
  ON public.tenant_applications
  FOR UPDATE
  TO authenticated
  USING (
...
CREATE POLICY IF NOT EXISTS tenant_applications_user_select_own
  ON public.tenant_applications
  FOR SELECT
  TO authenticated
  USING (
