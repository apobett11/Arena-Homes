...
-- ============================================================================
-- PART 2: CORE TABLES - PROFILES & ROLES
-- ============================================================================
...
-- Profiles table - extended with universal columns
...
UPDATE public.profiles SET
  ...
  role_id = COALESCE(role_id, UPPER(role))
WHERE display_name IS NOT NULL OR phone IS NOT NULL OR role IS NOT NULL;

-- ============================================================================
-- PART 3: CORE TABLES - EMPLOYEES
-- ============================================================================
...
-- ============================================================================
-- PART 4: CORE TABLES - PROPERTIES
-- ============================================================================

-- Ensure properties table has all universal columns
...
ALTER TABLE public.properties ADD CONSTRAINT properties_listing_status_check
  CHECK (listing_status IN ('DRAFT', 'PUBLISHED', 'HIDDEN', 'ARCHIVED'));

-- Backfill caretaker relationships
UPDATE public.properties SET
  caretaker_user_id = COALESCE(caretaker_user_id, caretaker_id),
...
-- ============================================================================
-- PART 5: CORE TABLES - UNITS
-- ============================================================================

-- Ensure units table has all universal columns
ALTER TABLE public.units
  ...
  ADD COLUMN IF NOT EXISTS deposit_amount numeric(12, 2),
  ADD COLUMN IF NOT EXISTS availability_status public.unit_availability_status NOT NULL DEFAULT 'AVAILABLE',
  ADD COLUMN IF NOT EXISTS is_public boolean NOT NULL DEFAULT true,
  ADD COLUMN IF NOT EXISTS photos text[] DEFAULT '{}'::text[],
  ADD COLUMN IF NOT EXISTS amenities jsonb DEFAULT '{}'::jsonb,
  ADD COLUMN IF NOT EXISTS last_updated timestamptz DEFAULT now();

-- Backfill room_number from type if missing
UPDATE public.units SET room_number = type WHERE room_number IS NULL AND type IS NOT NULL;

-- Generate proper room numbers for units without them
UPDATE public.units SET room_number = 'R-' || LPAD(id::text, 3, '0')
WHERE room_number IS NULL OR room_number = type;

-- ============================================================================
-- PART 6: CORE TABLES - TENANTS
-- ============================================================================

-- Ensure tenants table has all universal columns
...
-- Update tenant status column type if needed (migrate from old enum)
ALTER TABLE public.tenants
  ALTER COLUMN status TYPE text,
  ALTER COLUMN status SET DEFAULT 'ACTIVE';

-- Backfill tenant data from related tables
...
-- Backfill property_id, unit_id from active leases
...
-- Backfill caretaker relationships from properties
UPDATE public.tenants SET
...
-- Backfill room_number from units
...
-- ============================================================================
-- PART 7: CORE TABLES - LEASES
-- ============================================================================

-- Ensure leases table has all universal columns
ALTER TABLE public.leases
  ADD COLUMN IF NOT EXISTS lease_number text UNIQUE,
  ADD COLUMN IF NOT EXISTS property_id uuid REFERENCES public.properties(id),
  ADD COLUMN IF NOT EXISTS auto_renew boolean DEFAULT true;

-- Backfill property_id from units
UPDATE public.leases SET property_id = (
  SELECT u.property_id FROM public.units u WHERE u.id = leases.unit_id
) WHERE property_id IS NULL;

-- Generate lease numbers for existing leases without them
UPDATE public.leases SET lease_number = 'LS-' || UPPER(SUBSTRING(id::text, 1, 8))
WHERE lease_number IS NULL;

-- ============================================================================
-- PART 8: CORE TABLES - PAYMENTS
-- ============================================================================

-- Ensure payments table has all universal columns
...
-- Backfill property_id and unit_id from leases
UPDATE public.payments SET
  property_id = COALESCE(property_id, (
    SELECT l.property_id FROM public.leases l WHERE l.id = payments.lease_id
  )),
  unit_id = COALESCE(unit_id, (
    SELECT l.unit_id FROM public.leases l WHERE l.id = payments.lease_id
  ));

-- Backfill from tenants if still missing
UPDATE public.payments SET
  property_id = COALESCE(property_id, (
    SELECT t.property_id FROM public.tenants t WHERE t.id = payments.tenant_id
  )),
  unit_id = COALESCE(unit_id, (
    SELECT t.unit_id FROM public.tenants t WHERE t.id = payments.tenant_id
  ))
WHERE property_id IS NULL OR unit_id IS NULL;

-- ============================================================================
-- PART 9: RELATED TABLES - ISSUES (updated schema)
-- ============================================================================

-- Update issues table to universal schema
...
UPDATE public.issues SET status = 'PENDING' WHERE status = 'OPEN';
UPDATE public.issues SET status = 'CLOSED' WHERE status = 'RESOLVED' AND status NOT IN ('PENDING', 'IN_PROGRESS', 'RESOLVED', 'ESCALATED', 'CLOSED');

-- Backfill tenant_id from reporter_id
UPDATE public.issues SET
  tenant_id = COALESCE(tenant_id, (
    SELECT t.id FROM public.tenants t WHERE t.user_id = issues.reporter_id
  )),
  tenant_user_id = COALESCE(tenant_user_id, reporter_id);

-- Backfill property_id from unit_id
...
-- Create repairs from maintenance_requests
...
-- ============================================================================
-- PART 11: TABLE - ANNOUNCEMENTS (updated schema)
-- ============================================================================
...
-- Migrate content column to body if body doesn't exist
DO $$
BEGIN
  IF EXISTS (
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE public.announcements ADD COLUMN body text NOT NULL DEFAULT '';
  END IF;
END $$;

-- Backfill sender data
UPDATE public.announcements SET
  sender_user_id = COALESCE(sender_user_id, author_id),
  is_published = COALESCE(is_active, true);

-- ============================================================================
-- PART 12: NEW TABLE - MESSAGES
-- ============================================================================
...
-- ============================================================================
-- PART 13: NEW TABLE - PROPERTY_REVIEWS
-- ============================================================================
...
-- Migrate from tenant_comments if exists
...
WHERE property_id IS NOT NULL
  AND NOT EXISTS (
    ...
    WHERE property_reviews.tenant_id = tenant_comments.tenant_id 
    AND property_reviews.property_id = tenant_comments.property_id
  );

-- ============================================================================
-- PART 14: NEW TABLE - PROPERTY_LIKES
-- ============================================================================
...
-- ============================================================================
-- PART 15: NEW TABLE - PROPERTY_INVENTORY
-- ============================================================================
...
-- ============================================================================
-- PART 16: NEW TABLE - PROPERTY_FACILITIES (detailed)
-- ============================================================================
...
WHERE p.facilities IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM public.property_facilities 
    WHERE property_facilities.property_id = p.id
  );

-- ============================================================================
-- PART 17: TABLE - TENANT_APPLICATIONS (updated schema)
-- ============================================================================

-- Create new applications table with universal schema if not exists
...
-- Migrate from old tenant_applications if exists
...
WHERE NOT EXISTS (SELECT 1 FROM public.tenant_applications WHERE tenant_applications.id = tenant_applications_old.id)
ON CONFLICT (id) DO NOTHING;

-- ============================================================================
-- PART 18: TABLE - SUSPICIOUS_REPORTS (updated schema)
-- ============================================================================
...
UPDATE public.suspicious_reports SET status = 'OPEN' WHERE status = 'PENDING';
UPDATE public.suspicious_reports SET status = 'REVIEWING' WHERE status = 'INVESTIGATING';

-- ============================================================================
-- PART 19: NEW TABLE - SITE_SETTINGS
-- ============================================================================
...
-- ============================================================================
-- PART 20: HELPER FUNCTIONS (SECURITY DEFINER)
-- ============================================================================
...
-- ============================================================================
-- PART 21: DATABASE VIEWS FOR FRONTEND
-- ============================================================================

-- View 1: Tenant Dashboard View
...
SELECT 
  ...
  t.whatsapp_number AS tenant_whatsapp_number,
  ...
  COALESCE(
    (SELECT SUM(py.months_covered) FROM public.payments py 
     WHERE py.tenant_id = t.id AND py.status = 'SUCCESS'), 0
  ) AS paid_months,
  t.move_in_date,
  t.move_out_date,
  (SELECT COUNT(*) FROM public.issues i WHERE i.tenant_id = t.id AND i.status = 'PENDING') AS pending_issues_count,
  (SELECT COUNT(*) FROM public.issues i WHERE i.tenant_id = t.id AND i.status = 'RESOLVED') AS resolved_issues_count,
  (SELECT COUNT(*) FROM public.repairs r WHERE r.tenant_id = t.id AND r.status IN ('PENDING', 'IN_PROGRESS')) AS pending_repairs_count,
  (SELECT COUNT(*) FROM public.repairs r WHERE r.tenant_id = t.id AND r.status = 'SOLVED') AS solved_repairs_count,
  ...
  (SELECT COUNT(*) FROM public.announcements a 
   WHERE a.is_published = true 
     AND (a.is_global = true OR a.property_id = t.property_id OR a.target_role = 'TENANT')) AS announcements_count,
...
-- View 3: Public Properties View (for listings page)
...
SELECT 
  ...
  (SELECT COUNT(*) FROM public.issues i WHERE i.property_id = e.assigned_property_id AND i.status = 'PENDING') AS pending_issues_count,
  (SELECT COUNT(*) FROM public.issues i WHERE i.property_id = e.assigned_property_id AND i.status = 'RESOLVED') AS resolved_issues_count,
  (SELECT COUNT(*) FROM public.repairs r WHERE r.property_id = e.assigned_property_id AND r.status IN ('PENDING', 'IN_PROGRESS')) AS pending_repairs_count,
  (SELECT COUNT(*) FROM public.repairs r WHERE r.property_id = e.assigned_property_id AND r.status = 'SOLVED') AS solved_repairs_count,
  (SELECT COUNT(*) FROM public.tenant_applications ta WHERE ta.property_id = e.assigned_property_id AND ta.status = 'PENDING') AS pending_applications_count,
  (SELECT COUNT(*) FROM public.announcements a WHERE a.sender_employee_id = e.id) AS outgoing_announcements_count,
  (SELECT COUNT(*) FROM public.announcements a 
   WHERE a.target_role = 'CARETAKER' 
     OR (a.property_id = e.assigned_property_id AND a.is_published = true)) AS incoming_announcements_count
FROM public.employees e
LEFT JOIN public.properties p ON p.id = e.assigned_property_id
WHERE e.role_id = 'CARETAKER' AND e.status = 'ACTIVE';

-- ============================================================================
-- PART 22: TRIGGERS FOR UPDATED_AT
-- ============================================================================
...
-- ============================================================================
-- PART 23: ENABLE RLS ON ALL TABLES
-- ============================================================================
...
CREATE POLICY tenants_select_own_or_staff ON public.tenants FOR SELECT
  USING (
    ...
    OR (
      public.is_caretaker() 
      AND property_id = public.current_assigned_property_id()
    )
  );
...
CREATE POLICY units_select_accessible ON public.units FOR SELECT
  USING (
    ...
    OR EXISTS (
      SELECT 1 FROM public.tenants t 
      WHERE t.unit_id = id AND t.user_id = auth.uid()
    )
  );
...
CREATE POLICY announcements_select_published ON public.announcements FOR SELECT
  USING (
    ...
    AND (
      is_global = true
      OR property_id = public.current_assigned_property_id()
...
-- PROPERTY_FAQS policies (same as rules)
...
CREATE POLICY property_reviews_select_public ON public.property_reviews FOR SELECT
  USING (true);
...
CREATE POLICY property_reviews_insert_tenant ON public.property_reviews FOR INSERT
  WITH CHECK (
    ...
    AND EXISTS (
      SELECT 1 FROM public.tenants t 
      WHERE t.id = tenant_id AND t.property_id = property_reviews.property_id
    )
  );
...
CREATE POLICY property_likes_select_public ON public.property_likes FOR SELECT
  USING (true);
...
-- PROPERTY_FACILITIES policies (same as inventory)
...
-- ============================================================================
-- PART 26: GRANT PERMISSIONS ON VIEWS
-- ============================================================================
...
-- ============================================================================
-- PART 27: VALIDATION QUERY FUNCTION
-- ============================================================================
...
AS $$
  SELECT jsonb_build_object(
    ...
    'relationship_counts', jsonb_build_object(
      'tenants_with_property_id', (SELECT COUNT(*) FROM public.tenants WHERE property_id IS NOT NULL),
      'tenants_with_unit_id', (SELECT COUNT(*) FROM public.tenants WHERE unit_id IS NOT NULL),
      'tenants_with_room_number', (SELECT COUNT(*) FROM public.tenants WHERE room_number IS NOT NULL),
      'tenants_with_caretaker_employee_id', (SELECT COUNT(*) FROM public.tenants WHERE caretaker_employee_id IS NOT NULL),
      'properties_with_caretaker_employee_id', (SELECT COUNT(*) FROM public.properties WHERE caretaker_employee_id IS NOT NULL),
      'caretakers_with_assigned_property_id', (SELECT COUNT(*) FROM public.employees WHERE role_id = 'CARETAKER' AND assigned_property_id IS NOT NULL)
    ),
    'helper_functions', jsonb_build_object(
      'current_user_role', EXISTS (SELECT 1 FROM information_schema.routines WHERE routine_schema = 'public' AND routine_name = 'current_user_role'),
      'is_admin', EXISTS (SELECT 1 FROM information_schema.routines WHERE routine_schema = 'public' AND routine_name = 'is_admin'),
      'is_caretaker', EXISTS (SELECT 1 FROM information_schema.routines WHERE routine_schema = 'public' AND routine_name = 'is_caretaker'),
      'is_tenant', EXISTS (SELECT 1 FROM information_schema.routines WHERE routine_schema = 'public' AND routine_name = 'is_tenant'),
      'current_employee_id', EXISTS (SELECT 1 FROM information_schema.routines WHERE routine_schema = 'public' AND routine_name = 'current_employee_id'),
      'current_tenant_id', EXISTS (SELECT 1 FROM information_schema.routines WHERE routine_schema = 'public' AND routine_name = 'current_tenant_id'),
      'current_assigned_property_id', EXISTS (SELECT 1 FROM information_schema.routines WHERE routine_schema = 'public' AND routine_name = 'current_assigned_property_id'),
      'can_read_property', EXISTS (SELECT 1 FROM information_schema.routines WHERE routine_schema = 'public' AND routine_name = 'can_read_property'),
      'can_manage_property', EXISTS (SELECT 1 FROM information_schema.routines WHERE routine_schema = 'public' AND routine_name = 'can_manage_property')
    )
  );
$$;
...
COMMENT ON MIGRATION IS 'Universal Database Contract - Arena Homes v1.0.0';
