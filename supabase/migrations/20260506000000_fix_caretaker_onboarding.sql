...
BEGIN
  ...
  RAISE NOTICE 'DEBUG: Initial auth check - v_caretaker_user_id: %, email: %',
    v_caretaker_user_id,
    p_caretaker_email;

  -- Generate password once. Same password goes to auth.users and return payload.
  ...
  RAISE NOTICE 'DEBUG: Generated password for caretaker email: %', p_caretaker_email;
...
INSERT INTO auth.users (
  ...
  ELSE
    RAISE NOTICE 'DEBUG: Auth user already exists: % - updating password and metadata', v_caretaker_user_id;
  ...
  RAISE NOTICE 'DEBUG: auth.identities INSERT/UPDATE completed';
  ...
  RAISE NOTICE 'DEBUG: Property created: %', v_property_id;
  ...
  RAISE NOTICE 'DEBUG: Caretaker employee created: %', v_caretaker_employee_id;
  ...
  IF p_rules IS NOT NULL AND jsonb_typeof(p_rules) = 'array' AND jsonb_array_length(p_rules) > 0 THEN
    ...
    LOOP
      IF v_rule_item->>'rule_text' IS NOT NULL AND TRIM(v_rule_item->>'rule_text') != '' THEN
  ...
  IF NOT EXISTS (
    ...
    RETURN jsonb_build_object(
      'success', false,
      'error', 'Auth identity verification failed',
      'caretaker_user_id', v_caretaker_user_id,
      'caretaker_email', p_caretaker_email
    );
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RETURN jsonb_build_object(
      'success', false,
      'error', 'Caretaker profile verification failed',
      'caretaker_user_id', v_caretaker_user_id,
      'caretaker_email', p_caretaker_email
    );
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RETURN jsonb_build_object(
      'success', false,
      'error', 'Caretaker employee verification failed',
      'caretaker_user_id', v_caretaker_user_id,
      'caretaker_employee_id', v_caretaker_employee_id
    );
  END IF;

  RAISE NOTICE 'DEBUG: Verification passed for caretaker user: %', v_caretaker_user_id;
  ...
  RAISE NOTICE 'DEBUG: Returning success - property_id: %, caretaker_user_id: %',
    v_property_id,
    v_caretaker_user_id;
